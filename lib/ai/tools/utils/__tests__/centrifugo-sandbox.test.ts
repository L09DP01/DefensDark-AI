...
describe("CentrifugoSandbox", () => {
  ...
  describe("files.write", () => {
    it("uses heredoc approach for text content", async () => {
      ...
      try {
        const sandbox = createSandbox();
        await sandbox.files.write("/tmp/defensdark-ai/test.txt", "hello world");
        ...
        expect(writeCmd.command).toContain("<<'DEFENSDARKAI_EOF_");
  ...
  describe("git-bash on Windows", () => {
    ...
    it("downloadFromUrl emits POSIX mkdir + curl with MSYS paths", async () => {
      ...
      await sandbox.files.downloadFromUrl(
        ...
        "/tmp/defensdark-ai-upload/image.png",
      );
      const cmd = runs[0];
      expect(cmd).toContain("mkdir -p '/c/temp/defensdark-ai-upload'");
      ...
      expect(cmd).toContain("-o '/c/temp/defensdark-ai-upload/image.png'");
    ...
    it("ensureDirectory emits mkdir -p with MSYS path", async () => {
      const { sandbox, runs } = createWindowsBashSandbox();
      await (sandbox as any).ensureDirectory("C:\\temp\\defensdark-ai-upload");
      expect(runs[0]).toBe("mkdir -p '/c/temp/defensdark-ai-upload'");
    });
    ...
    it("files.write for text content uses heredoc with MSYS path", async () => {
      ...
      expect(runs[1]).toContain("<<'DEFENSDARKAI_EOF_");
